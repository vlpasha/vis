...
Graph3d.DEFAULTS = {
  ...
  xTicks: autoByDefault,
  yTicks: autoByDefault,
  zTicks: autoByDefault
};
...
Graph3d.prototype._redrawAxis = function () {
  ...
  ctx.font = 20 / this.camera.getArmLength() + 'px arial';
  ...
  if (!(this.xStep instanceof Array)) {
    ...
    while (!step.end()) {
      ...
      if (this.showXAxis) {
        ...
        this.drawAxisLabelX(ctx, point3d, msg, armAngle, textMargin + 10);
      }

      step.next();
    }
  } else {
    this.xStep.forEach((x) => {
  ...
  if (!(this.yStep instanceof Array)) {
    ...
    while (!step.end()) {
      ...
      if (this.showYAxis) {
        ...
        this.drawAxisLabelY(ctx, point3d, msg, armAngle, textMargin + 10);
      }

      step.next();
    }
  } else {
    this.yStep.forEach((y) => {
      if (this.showGrid) {
        from = new Point3d(xRange.min, y, zRange.min);
        to = new Point3d(xRange.max, y, zRange.min);
        this._line3d(ctx, from, to, this.gridColor);
  ...
  if (this.showZAxis) {
    ...
    if (!(this.zStep instanceof Array)) {
  ...
  if (zLabel.length > 0 && this.showZAxis) {
    offset = 50; // pixels.  // TODO: relate to the max width of the values on the z axis?
    xText = (armVector.x > 0) ? xRange.min : xRange.max;
    yText = (armVector.y < 0) ? yRange.min : yRange.max;
